...
if [ ! -f "$ROOTDIR/coverage-$JSTD_VERSION.jar" ]; then
    ...
    curl http://js-test-driver.googlecode.com/files/coverage-$JSTD_VERSION.jar > coverage-$JSTD_VERSION.jar
fi

if [ ! -f "$ROOTDIR/JsTestDriver-$JSTD_VERSION.jar" ]; then
    echo "Downloading coverage jar ..."
    curl http://js-test-driver.googlecode.com/files/JsTestDriver-$JSTD_VERSION.jar > JsTestDriver-$JSTD_VERSION.jar
fi
